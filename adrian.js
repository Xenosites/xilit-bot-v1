...
module.exports = conn = async (conn, m, chatUpdate, store) => {
    try {
        ...
        const isCreator = [botNumber, global.owner, '6281255104397'].map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(m.sender)
  ...
  conn.sendMessage(m.chat, {
    ...
    contextInfo: {
      externalAdReply: {
        ...
        sourceUrl: "https://chat.whatsapp.com/GFvWy8hTOFM7a1gKOjJKLA",
        mediaType: 1,
...
( Q.S ${res.result.data.surah.name.transliteration.id} : ${res.result.data.number.inSurah} )`
		...
		case 'brainly':{
            ...
            let media = await conn.downloadMediaMessage(qmsg)
            let audio = await toAudio(media, 'mp4')
            conn.sendMessage(m.chat, {document: audio, mimetype: 'audio/mpeg', fileName: `Convert By Xenite.mp3`}, { quoted : m })
	        ...
	        case 'carbon':
	            ...
	            newReply(mess.wait)
	            conn.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/carbon?apikey=${apikey}&code=${q}&language=nodejs`}, caption: `Created By Xenite\n\n\nCode:\n\n${q}`}, {quoted: m})
	        break
		...
		case 'freefire':
            ...
            db.data.users[m.sender].limit -= 15
            newReply(`🚩 15 Limit Used`)
			if (args.length == 0) return newReply(`Example: ${prefix + command} Xenite `)
			newReply(mess.wait)
			conn.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/ephoto1/${command}?apikey=${apikey}&text=${args}`}, caption: `Created By Xenite\n\n Type: ${command}\n\nText: ${args}`}, { quoted: m })
			break
		...
		case 'carvedwood':
			...
			if (args.length == 0) return newReply(`Example: ${prefix + command} Xenite`)
			newReply(mess.wait)
			conn.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/photooxy1/${command}?apikey=${apikey}&text=${args}`}, caption: `Created By Xenite\n\n Type: ${command}\n\nText: ${args}`}, { quoted: m})
			break
    ...
    // Text Prome
		...
		case 'wallgravity':
			...
			if (args.length == 0) return newReply(`Example: ${prefix + command} Xenite`)
			newReply(mess.wait)
			conn.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/textprome2/${command}?apikey=${apikey}&text1=${args[0]}&text2=${args[1]}`}, caption: `Created By Xenite\n\n Type: ${command}\n\nText: ${args}`}, { quoted: m })
			break
		...
		case 'thunder':
			...
			if (args.length == 0) return newReply(`Example: ${prefix + command} Xenite`)
			newReply(mess.wait)
			conn.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/textprome/${command}?apikey=${apikey}&text=${args}` }, caption: `Created By Xenite\n\n Type: ${command}\n\nText: ${args}`}, { quoted: m })
			break
        ...
        break
			...
			case 'nomerhoki': case 'nomorhoki': {
                ...
                db.data.users[m.sender].limit -= 5
                newReply(`🚩 5 Limit Used`)
                if (!Number(text)) return newReply(`Contoh : ${prefix + command} 6281255104397`)
